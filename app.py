...
st.set_page_config(
    page_title="TechSevaRecruitment Agent.AI",
    page_icon="🤖",
    layout="wide"
)
...
if encoded_logo:
    st.sidebar.markdown(
        ...
        <div class='sidebar-logo'>
            ...
            <div style='color:#00c6ff;font-size:1.1em;font-family:sans-serif;font-weight:bold;text-shadow:0 1px 6px #ffd200;margin-top:8px;'>TechSeva.AI</div>
        </div>
        ...
        <div class='sidebar-logo' style='margin-top:0;'>
            ...
            <div style='color:#00c6ff;font-size:1.1em;font-family:sans-serif;font-weight:bold;text-shadow:0 1px 6px #ffd200;margin-top:8px;'>TechSevaRecruitment</div>
        </div>
        """,
        unsafe_allow_html=True
    )
    # Developer info and image below the logos
    st.sidebar.markdown("<div style='text-align:center;font-size:1.1em;margin-top:10px;'>👨👨‍💻<b>Developer:</b> Abhishek💖Yadav</div>", unsafe_allow_html=True)
    developer_path = os.path.join(os.path.dirname(__file__), "pic.jpg")
    if os.path.exists(developer_path):
        st.sidebar.image(developer_path, caption="Abhishek Yadav", use_container_width=True)
    else:
...
def analyze_resume(agent, resume_file, role, custom_jd):
    ...
    # Defensive: Check file size (max 2MB)
    if hasattr(resume_file, 'size') and resume_file.size > 2 * 1024 * 1024:
        st.error("⚠️ Resume file too large (max 2MB). Please upload a smaller file.")
        return None
    ...
    if hasattr(resume_file, 'name'):
        ...
        if ext not in ["pdf", "txt"]:
            st.error("⚠️ Only PDF or TXT resumes are supported.")
            return None

    # Defensive: Check API key
    if not agent.api_key or len(agent.api_key.strip()) < 20:
        st.error("⚠️ Invalid or missing OpenAI API key. Please check your key in the sidebar.")
        return None

    try:
        with st.spinner("🔍 Analyzing resume... This may take a minute."):
            if custom_jd:
                # Defensive: Check JD file size (max 2MB)
                if hasattr(custom_jd, 'size') and custom_jd.size > 2 * 1024 * 1024:
                    st.error("⚠️ Job description file too large (max 2MB). Please upload a smaller file.")
                    return None
                # Defensive: Check JD file type
                if hasattr(custom_jd, 'name'):
                    ext_jd = custom_jd.name.split('.')[-1].lower()
                    if ext_jd not in ["pdf", "txt", "xml"]:
                        st.error("⚠️ Only PDF, TXT, or XML job descriptions are supported.")
                        return None
...
def main():
    ...
    with tabs[0]:
        # Custom job description upload (text, pdf, xml)
        ...
        custom_jd = None
        if upload_jd:
            custom_jd_file = st.file_uploader("Upload job description (PDF, TXT, XML)", type=["pdf", "txt", "xml"])
            if custom_jd_file:
